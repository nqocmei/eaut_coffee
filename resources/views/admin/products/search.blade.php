...
@section('admin_content')
    ...
    <div class="d-flex justify-content-between">
        ...
        <form action="{{route('adminSearch')}}" method="GET" class="d-flex">
            <input type="text" value="" placeholder="Nhập để tìm kiếm..." name="keyword" class="form-control" style="width: unset;" required>
    ...
    <nav aria-label="Page navigation example">
        <ul class="pagination">
            ...
            <li class="page-item @if($searchs->currentPage() === 1) disabled @endif">
                <a class="page-link" href="{{ $searchs->previousPageUrl() }}&keyword={{ $keyword }}">Previous</a>
            </li>
            @for ($i = 1; $i <= $searchs->lastPage(); $i++)
                <li class="page-item @if($searchs->currentPage() === $i) active @endif">
                    <a class="page-link" href="{{ $searchs->url($i) }}&keyword={{ $keyword }}">{{ $i }}</a>
                </li>
            @endfor
            ...
            <li class="page-item @if($searchs->currentPage() === $searchs->lastPage()) disabled @endif">
                <a class="page-link" href="{{ $searchs->url($searchs->lastPage()) }}&keyword={{ $keyword }}">Last</a>
            </li>
        </ul>
    </nav>
